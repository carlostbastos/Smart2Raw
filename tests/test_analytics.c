...
int main(void){
    printf("=== #1 LARGURA BIDIRECIONAL (auto-cura) ===\n");
    {
        ...
        CHECK(s2r_abs_size(p.size)==64, "outlier promoted u8->u64");
        size_t bytes_u64 = p.byte_cap;
        /* remove the outlier (it's at the end) */
        s2r_remove_swap(&p, p.count-1);
        CHECK(s2r_max(&p)==60, "apos remover, max volta a 60");
        /* self-healing: lowers */
        s2r_fit_class(&p);
        CHECK(s2r_abs_size(p.size)==8, "fit_class lowered u64->u8");
    ...
    {   /* signed bidirecional */
        ...
        CHECK(s.size==S2R_I32, "signed: I8->I32 com outlier");
    ...
    {   /* tracked: range O(1) */
        ...
        CHECK(mn==s2r_min(&t.p)&&mx==s2r_max(&t.p), "tracked range O(1) bate com rescan");
    ...
    printf("\n=== #3 GROUP-BY direto no formato compacto ===\n");
    {
        ...
        /* COUNT(*) : histogram; validates against the naive reference and measures */
        ...
        printf("  histogram 10M u8: naive=%.2f ms (%.0f M/s)  4-way=%.2f ms (%.0f M/s)  %.1fx\n",
