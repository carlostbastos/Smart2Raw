 ...
 */
/* Integrated Smart2Raw v3.3.0 test */
...
int main(void){
    printf("Smart2Raw %s  | AVX2 runtime: %s | mmap: %d | stdio: %d\n\n",
        S2R_VERSION_STRING, s2r_has_avx2()?"sim":"nao", S2R_HAS_MMAP, S2R_HAS_STDIO);

    /* 1. self-adaptive push: grows the type by itself */
    printf("1. push_adaptive\n");
    {
        S2RPool p; s2r_pool_init(&p,S2R_8,4);
        s2r_push_adaptive(&p,200);        /* fits u8 */
        s2r_push_adaptive(&p,5000);       /* forces u16 */
        s2r_push_adaptive(&p,100000);     /* forces u32 */
        s2r_push_adaptive(&p,5000000000ULL); /* forces u64 */
        ...
        s2r_push_signed_adaptive(&s,-30000);   /* forces I16 */
        s2r_push_signed_adaptive(&s,-2000000000);/* forces I32 */
    ...
    /* 2. SIMD sum == scalar sum (any dispatch path) */
    ...
    /* 4. I/O portavel + CRC + mmap zero-copy */
    printf("4. FS portavel + mmap\n");
    {
        ...
        CHECK(e==S2R_OK, "map_open com verificacao de CRC");
        if(e==S2R_OK){
            ...
            /* operate directly on the mapping */
    ...
    /* 5. mmap detects a corrupt file */
#if S2R_HAS_MMAP
    printf("5. mmap + CRC detects corruption\n");
    {
    ...
    printf("6. promotion on an empty pool does not overflow (count==0 regression)\n");
    {
        /* small init; first value already needs a wider class; fill up to the initial capacity */
        ...
        CHECK(s2r_used_bytes(&p)==8000, "signed: bytes consistentes (1000*8)");
        s2r_pool_free(&p);
