 ...
 * in-place swap. Here we prove this does NOT modify the on-disk file (COW),
 * which is the safety guarantee a big-endian host depends on.
 * The correctness of the VALUES on BE depends on s2r_swap_payload (tested both
 * ways) + CRC-before-swap; that does not run on LE hardware. */
...
int main(void){
    ...
    /* replicates EXACTLY what the BE branch does: MAP_PRIVATE + mprotect + in-place swap */
    ...
    s2r_swap_payload(payload,4,bytes/4);                 /* dirta paginas (COW), nao o arquivo */
    /* verify that after the swap the bytes in MEMORY changed */
    CHECK(memcmp(payload,orig+16,bytes)!=0,"swap changed the in-memory copy (COW)");
    munmap(base,(size_t)sz); close(fd);

    /* RE-READ the on-disk file: must be INTACT */
    f=fopen(path,"rb"); unsigned char*after=malloc((size_t)sz); fread(after,1,(size_t)sz,f); fclose(f);
    CHECK(memcmp(orig,after,(size_t)sz)==0,"arquivo em disco INALTERADO apos COW+swap");
    ...
    CHECK(memcmp(orig,ref,(size_t)sz)==0,"swap duplo e involutivo (LE<->BE round-trip)");

    /* regression: the LE path (zero-copy) stays correct */
    S2RMap m;
    ...
    CHECK(eq,"LE values match (zero-copy)");
    s2r_map_close(&m);
