...
int main(void){
    /* 1) signed PFOR: round-trip, sum, and recovery under outliers */
    {
        ...
        size_t single_w=4*n; /* I32 in the single width */
    ...
    /* 2) accelerated block sum: == scalar sum, and measures throughput */
    {
        ...
        printf("  block sum: SIMD=%.2f ms (%.2f G/s)  scalar=%.2f ms (%.2f G/s)  %.2fx\n",
