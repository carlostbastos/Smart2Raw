...
#define S2R_VERSION_MINOR 4
#define S2R_VERSION_PATCH 0
#define S2R_VERSION_STRING "3.4.0"
...
#define S2R_BLK_FMT    2u     /* format version of a serialized blocked column */
...
typedef struct {
    ...
    void    *bspan;     /* true max delta of each block, class pcls         */
    void    *bsum;      /* sum of each block, class scls                    */
    int8_t   bcls, ocls, pcls, scls;
    size_t   nblocks;
...
static inline void s2r_blocked_free(S2RBlocked *b){
    ...
    free(b->boff); free(b->bbase); free(b->bspan); free(b->bsum);
    memset(b,0,sizeof *b);
}
...
static inline int s2r__blocked_build(S2RBlocked *b, const uint64_t *su, const int64_t *ss,
    ...
    if(!tbase||!tspan||!tsum||!toff||!b->bclass||!b->bflags){
        free(tbase);free(tspan);free(tsum);free(toff); s2r_blocked_free(b); return 0;
    }

    size_t total=0;
    int64_t  lo_b=0, hi_b=0;                 /* range of the bases */
    uint64_t hi_p=0;                         /* largest span       */
    ...
    b->bcls = is_signed ? s2r_classify_signed_range(lo_b,hi_b) : (int8_t)s2r_classify((uint64_t)hi_b);
    ...
    b->scls = is_signed ? s2r_classify_signed_range(lo_s,hi_s)
                        : (int8_t)s2r_classify((uint64_t)hi_s);
    ...
    if(!b->bbase||!b->bspan||!b->boff||!b->bsum){
        free(tbase);free(tspan);free(tsum);free(toff); s2r_blocked_free(b); return 0;
    }
    for(size_t bi=0;bi<b->nblocks;bi++){
    ...
    if(total){
        ...
        if(!b->data){ free(tbase);free(tspan);free(tsum);free(toff); s2r_blocked_free(b); return 0; }
    }
    b->bytes=total;
    ...
    for(size_t bi=0; bi<b->nblocks; bi++){
        ...
        if(!w) continue;
        size_t start=bi*block, len=(start+block<=n)?block:(n-start);
        uint64_t base=(uint64_t)tbase[bi];
    ...
    free(tbase);free(tspan);free(tsum);free(toff);
    return 1;
}
...
static inline uint64_t s2r_blocked_get(const S2RBlocked *b, size_t i){
    ...
    size_t bi=i/b->block;
    return s2r__bb(b,bi) + s2r__blocked_delta(b,bi,i%b->block);
}
static inline int64_t s2r_blocked_get_signed(const S2RBlocked *b, size_t i){
    if(!b || i>=b->count) return 0;
    size_t bi=i/b->block;
    return (int64_t)(s2r__bb(b,bi) + s2r__blocked_delta(b,bi,i%b->block));
}
...
static inline uint64_t s2r_blocked_max(const S2RBlocked *b){
    ...
    uint64_t m=s2r__bb(b,0)+s2r__bp(b,0);
    for(size_t bi=1; bi<b->nblocks; bi++){
        uint64_t x=s2r__bb(b,bi)+s2r__bp(b,bi);
        if(x>m) m=x;
    }
    return m;
}
...
static inline uint8_t *s2r__blk_stage(const S2RBlocked *b, size_t *out){
    ...
    struct { const void *a; int8_t c; int sg; } arr[4] = {
...
static inline S2RError s2r_blocked_save(const S2RBlocked *b, const char *path){
    ...
    uint8_t hdr[4]={ 0, (uint8_t)(b->is_signed?S2R_FLAG_SIGNED:0), (uint8_t)S2R_BLK_FMT, 0 };
...
static inline S2RError s2r_blocked_load(S2RBlocked *b, const char *path){
    ...
    if(hdr[0]!=0||hdr[2]!=(uint8_t)S2R_BLK_FMT||hdr[3]!=0){ fclose(f); return S2R_ERR_CORRUPT; }
    ...
    {   struct { void *a; int8_t c; int sg; } arr[4] = {
