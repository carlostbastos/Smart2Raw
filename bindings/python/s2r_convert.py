...
Same cycle as s2r_convert.c, driven by Python over the shared lib:
CONVERT (text -> compact, classify) -> PROCESS in place on the compact data
(safe arithmetic, no truncation) -> UNCONVERT (compact -> text). The overflow
ceiling is --cap (default 32): if the operation would exceed the ceiling, it REFUSES.
...
def main():
    ...
    # (2) PROCESS with the --cap overflow ceiling
    if a.op != "none" and len(p) > 0:
        mx = abs(p.max())
        proj = mx + a.by if a.op == "add" else mx * a.by
        ceiling = (1 << 64) - 1 if a.cap >= 64 else (1 << 32) - 1
        if abs(proj) > ceiling:
    ...
    sys.stderr.write(
        ...
        f"[op={a.op} by={a.by} cap={a.cap}] -> unconverted ({abs(cls_out)} bits) -> {a.out}\n")
