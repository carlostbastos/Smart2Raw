...
| `feature_store.c`  | each column at its own width; total vs uniform int64 |
...
| `ai_zeropoint.c`   | per-block zero-point correction (row-sum), exact |
| `indices.c`        | token IDs: 2x if vocab <=16b, ~1x above (honest) |

Honest yardstick: the AI examples win vs the *safe* uniform width (u16
forced by outliers), not vs a flat int8; and none of this is the GEMM itself.
