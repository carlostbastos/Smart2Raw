 ...
 */
/* AI - zero-point correction (row-sum) of int8 GEMM, summed per block (SIMD).
 * Here we validate EXACTNESS; the speedup (~6.7x) is in benchmarks/. */
#include <stdio.h>
#include <stdlib.h>
#include "smart2raw.h"
int main(void){
    ...
    S2RBlocked b; s2r_blocked_build(&b, w, N, K);                    /* 1 bloco por linha */
    uint64_t fast = s2r_blocked_sum_fast(&b), slow = s2r_blocked_sum(&b);
    printf("ai_zeropoint: int8 weights %zux%zu, block sum (vpsadbw per block)\n", M, K);
