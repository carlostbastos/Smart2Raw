 ...
 * Honest: byte-granular does not beat flat int8; the gain is vs the safe uniform
 * width (u16, forced by outliers) WHEN the outlier is localized. */
#include <stdio.h>
#include <stdlib.h>
#include "smart2raw.h"
int main(void){
    ...
    size_t unif = 2*N;                              /* u16 seguro */
    printf("ai_activations: %.1f%% outlier channels, per-channel layout\n", 100.0*nout/C);
    printf("  uniform-u16 = %.1f MB | per block = %.1f MB -> %.2fx smaller (exact sum)\n",
