...
# Expected result: 31 suites OK, 0 failures.
set -u
...
run "Fuzz diferencial (101k)"                  -O3 -march=native test_fuzz_diff.c
run "Fatoracao afim base+passo (283)"          -O3 -march=native test_affine.c
run "Ordem/sinal/constante/predicados (196)"   -O3 -march=native test_gaps.c
run "Resumo/indice/planejador (125)"           -O3 -march=native test_summary.c
run "Ordem: busca binaria forcada off"         -O2 -DS2R_BLK_BSEARCH_MIN_BYTES=999999999 test_gaps.c
run "Ordem: busca binaria forcada on"          -O2 -DS2R_BLK_BSEARCH_MIN_BYTES=0 test_gaps.c
